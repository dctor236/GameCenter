...
export default class Move extends mw.Script {
    ...
    initTransLate() {
        ...
        //一秒多少  最终终点是  
        let strPos = this.gameObject.worldTransform.position.clone();
        let endPos = this.gameObject.worldTransform.position.clone().add(this.translateDistance);

        if (this.translateRepeat) {
            this.tween = new Tween({ time: 0 }).to({ time: 1 }, this.translateTime * 1000).onUpdate((obj) => {
                this.gameObject.worldTransform.position = mw.Vector.lerp(strPos, endPos, obj.time);
        ...
        Event.addLocalListener(Move.MoveEventTitle, (isPlay: boolean) => {
            isPlay ? this.tween.start() : this.tween.stop();
            if (isPlay) {
