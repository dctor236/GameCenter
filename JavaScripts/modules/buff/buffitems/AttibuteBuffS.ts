...
export class AttibuteBuffS extends BuffS {
    ...
    protected onExcete(): void {
        super.onExcete()
        if (!this.host) return
        const guid = this.hostGuid
        let target: any = MonsterMgr.Inst.getMonster(guid)
        if (!target) {
            target = PlayerMgr.Inst.getPlayer(guid)
        }
        let newAtt: number = 0
        ...
        console.log("启动金身", target.getAtt(this.config.affectProperty))
   ...
   */
    public onDestroy() {
        ...
        if (target instanceof Monster) {
            lastAtt = FightMgr.instance.getMAttibuteList(guid).getValue(this.config.affectProperty)
        } else if (target instanceof TsPlayer) {
            lastAtt = FightMgr.instance.getPAttibuteList(guid).getValue(this.config.affectProperty)
        }
        target.setAtt(this.config.affectProperty, lastAtt)
        console.log("关闭金身", target.getAtt(this.config.affectProperty))
        super.onDestroy();
    }
