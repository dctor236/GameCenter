...
export default class FacadMainUI extends FacadMain_Generate {
    ...
    protected onStart(): void {
        ...
        const data = DataCenterC.getData(ShopDataInfo)
        this.mGold.text = data.getCoin(CoinType.PeaCoin).toString();
        data.onSilverCoinChange.add((num: number) => {
        ...
        this.mBtnBuy.onClicked.add(() => {
            ...
            const config = GameConfig.RoleAvatar.getElement(this._curSelect)
            const nightData = DataCenterC.getData(ShopDataInfo)
            if (config.price > nightData.getCoin(CoinType.PeaCoin)) {
                Tips.show("钻石不足")
            } else {
