...
export class ActionModuleC extends ModuleBaseC {
    ...
    public foreceActionWithNpc(configId: number, npcID: number): void {
        ...
        if (config.doubleType != ActionDoubleType.Ordinary) {
            ...
            this.curInterectNPCData.start(this.currentPlayer, npc.client, config);
...
export class InteractPlayer {
    ...
    public start(player1: mw.Player, player2: mw.Player | NPCClient, config: ActionBaseCfg) {
        ...
        if (player2 instanceof mw.Player) {
            ...
            GeneralManager.modiftEnterInteractiveState(this.interactObj, char2).then((suc: boolean) => {
                ...
                PlayerManagerExtesion.changeStanceExtesion(char2, config.accectStance)
    ...
    public leave(): void {
        ...
        if (!accectPlayer) {
            let npcClinet = ModuleService.getModule(NPCModule_C).getNpc(this.accectPlayerId).client
            ...
            GeneralManager.modifyExitInteractiveState(this.interactObj, char2.worldTransform.position.add(char2.getForwardVector().multiply(100)));
        }
        PlayerManagerExtesion.changeStanceExtesion(char2, "")
