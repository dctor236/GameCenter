...
* @LastEditTime: 2023-07-04 21:34:51
...
export default class DragonMonster extends MonsterBase {
    ...
    protected onIdle(...data: any): void {
        ...
        if (all && all.length > 0 && this.humanHeight == 0) {
            ...
            // this.initHeadLoc = this._head.worldTransform.position.clone()
            // this.initHeadRot = this._head.worldTransform.rotation.clone()
            // this.initDragonLoc = this.host.worldTransform.position.clone()
            // this.initDragonRot = this.host.worldTransform.rotation.clone()
        }
        this.firstIn = false
    ...
    private curSkillElem: IMonsterSkillElement = null
    ...
    protected onTarget(...data: any) {
        super.onTarget(data)
        Tips.showToClient(Player.getPlayer(this._curTagetPid), "火龙王似乎盯上你了!")
        if (this.curBehav != -1)
    ...
    protected targetPrepareUpdate(dt: number, eslapsed: number) {
        ...
        if (!this.flashStart) {
            if (eslapsed >= 1.5) {
                ...
                this.monsterStateCb(EMonserState.Hide)
            ...
            if (!this.flashEnd && eslapsed >= 3) {
                ...
                this.monsterStateCb(EMonserState.Visable)
                this.host.setVisibility(mw.PropertyStatus.On)
                this.host.setCollision(mw.CollisionStatus.QueryOnly)
                this._dragonTransp?.setVisibility(mw.PropertyStatus.Off)
                ...
                if (b.option && b.value && b.value.length > 0) {
                    for (let j = b.value.length - 1; j >= 0; j--) {
                        ...
                        if (this.aoeSkills[i].behaviorIndex == 3 && delta <= tmpVal) {
                            ...
                            console.log("hjkljkljklj1", this.preReases)
                            ...
                            if (delta >= tmpVal - 20 && delta <= tmpVal) {
                                this.aoeSkills[i].count = this.aoeSkills[i].count + 1
                                this.preReases.push(index)
                                console.log("hjkljkljklj2", this.preReases)
                            }
                        }
                    }
                }
    ...
    protected deletePlayer(playerId: number) {
        ...
        if (index >= 0) {
            ...
            Event.dispatchToLocal(EventsName.PlayerBattle, playerId, false)
    ...
    protected addPlayer(playerId: number) {
        ...
        if (index < 0) {
            ...
            Event.dispatchToLocal(EventsName.PlayerBattle, playerId, true)
        }
    }
