...
export class MonsterSkillBase {
    ...
    public playReleaseEffSound() {
        ...
        if (this._config.release_sound && this._config.release_sound != null) {
            ...
            const audioId = mw.SoundService.play3DSound(this._config.release_sound, pos,
                ...
                { radius: 15000, falloffDistance: 15000 });
        ...
        if (this._config.release_effect) {
            ...
            if (this.isMonster) {
                rot = new Rotation(this._atkVec.x, this._atkVec.y, this._atkVec.z)
            } else {
    ...
    public onStart() {
        ...
        const res = MonsterMgr.Inst.getMonster(this._hostID)
        if (res) {
            this.isMonster = true
        } else {
            ...
            hurtActors?.forEach(e => {
                ...
                if (e instanceof Monster) {
                    if (SystemUtil.isClient()) {
                        ...
                        let spiritData = FightMgr.instance.calculateSDamage(this._hostID, damage)
                        damage = spiritData.data
                        TS3.monsterMgr.onDamage(e.mid, damage, spiritData.crit)
                    } else {
                    ...
                    if (SystemUtil.isClient()) {
                        ...
                        if (this._config.buff && this._config.buff.length > 0) {
                            for (const buffId of this._config.buff) {
                                ...
                                if (canRelease)
                                    ModuleService.getModule(BuffModuleS).addABuff(e.userId, buffId, EBuffHostType.Player, true)
                            }
                        }
                        ...
                        if (this._config.damage && this._config.damage.length == 2) {
                            //计算怪物伤害
                            damage = MathUtil.randomInt(this._config.damage[0], this._config.damage[1])
                            TS3.playerMgr.onDamage(damage, e.userId)
                        }
                    }
                }
            })
    ...
    public cylinderFilter(shapeParams: any) {
        ...
        if (this.isMonster) {
            ...
            allActors = [MonsterMgr.Inst.getNearMonster(shapeParams[0], this.inspecterPos)]
        }
    ...
    public boxFiter(shapeParams: any) {
        ...
        let allActors = []
        if (this.isMonster) {
            allActors = PlayerMgr.Inst.getRangePlayer(shapeParams[0], "0", this.inspecterPos)
        } else {
            allActors = [MonsterMgr.Inst.getNearMonster(shapeParams[0], this.inspecterPos)]
        }
