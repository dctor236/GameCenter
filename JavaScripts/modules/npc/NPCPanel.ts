...
export class P_NPCPanel extends NPCPanel_Generate {
	...
	/**新手引导玩家的选项 */
	protected guideSelect: number = 0
	 ...
	 * @returns 
	 */
	public showNpcTalk(npcTalkID: number, isGuide: boolean, config?: ITalkEventElement) {
		if (config) {
		...
		if (!npcTalkID) {
			...
			this.talkTxt.text = GameUtils.getTxt(this.npctalkData.language);
			this.onTalkComplete();
