        ...
        taskList.forEach(e => {
            ...
            elem.TaskNPCID.forEach(npcID => {
                ...
                if (elem.accTalkState >= 0) {
                    accNpc.client.setTalkState(elem.accTalkState)
                }
                if (elem.clickTalkState) {
                    elem.clickTalkState.forEach(state => {
                        const id = state[0]
                        if (accNpc.client.trigger.checkInArea(ch) && GlobalData.CurInteractNpc != accNpc.client.npcID) {
            ...
            if (task.curSolveTime >= task.totalSolveTime) {
                ...
                elem.TaskNPCID.forEach(npcID => {
                    if (elem.finishTalkState >= 0) {
                        ...
                        if (finishNpc) {
                            finishNpc.client.setTalkState(0)
                        }
                    }
                })
            }
    ...
    public setGuidVisable(visable: boolean) {
        ...
        if (visable) {
            if (curData.taskState == EmTaskState.Doing) {
                ...
                UIManager.show(GuidePoint, [npc.client.npcObj])
            }
        }
        else {
