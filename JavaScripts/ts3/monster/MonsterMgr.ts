...
export class MonsterMgr {
    ...
    public async createMonstersObj() {
        ...
        for (const elem of elems) {
            let obj = await this.createMonsterByType(elem.monsterType, elem.Guid)
            let host = obj.getChildren()[0].getChildByName('头部').getChildByName('恐龙')
            let monster = this.register(host, {
    ...
    public getMonster(mid: string): Monster {
        if (this.isVoid)
            return null;
        if (SystemUtil.isClient())
            return this.moduleC.getObj(mid)
        else
            return this.moduleS.getObj(mid)
    }
    public getAllMonser() {
        if (this.isVoid)
            return null;
